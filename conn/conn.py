...
def plotMatsEEHoriz(): 
    ...
    for i,(label, preBin, postBin) in enumerate(zip(labels,labelPreBins, labelPostBins)):
        ...
        if i==0:
            ...
            binsPost.append(list(bins[('W+AS', 'PT', 'L5B')][-1]))
            smatLabel=[[y for y in x] for x in smatLabel]
            smatLabel.append([0]*len(smatLabel[0]))
            ...
            binsPost.insert(1,list(bins[('W+AS', 'IT', 'L5A,5B')][1]))
            smatLabel=[[y for y in x] for x in smatLabel]
    ...
    fig.set_figheight(8)
    fig.set_figwidth(15)
    fig.savefig(dataFolder+'connEE_smat_hor.png', dpi=1000)
