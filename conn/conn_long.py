...
def plotMats2D(labels=0, cbar=0):
    ...
    EorIvals = ['exc', 'inh']
    # cmat
    ybinsNew = np.arange(0, 1.0, 0.025)
    rc('font',**{'family':'sans-serif','sans-serif':['Arial']})
    matplotlib.rcParams.update({'font.size': 16}) #, 'font.weight':'bold'})
    plt.set_cmap('jet')
    fontsiz=26
    
    maxVal = max([max(v) for k,v in cmat.iteritems()])
    ...
    for EorI in ['exc']:#EorIvals:
        ...
        for isubplot, ct in enumerate(cellTypes):
            ...
            mat2d = []
            for longPop in longPops:
                conv = [cmat[(longPop,ct,EorI)][0]]+cmat[(longPop,ct,EorI)]+[cmat[(longPop,ct,EorI)][-1]]
                ybins = [min(bins[(longPop, ct)][0])]+[mean(x) for x in bins[(longPop, ct)]]+[max(bins[(longPop, ct)][-1])]
                #print longPop, ybins
                #print ybins, conv

                f = interp1d(ybins, conv, fill_value=(0, 0), bounds_error=False, kind='nearest')
                convNew = f(ybinsNew)
                mat2d.append(convNew)
...
if fixedNumSyns:
    meanNumSyns = mean(numSyns['Meyer'].values())
...
if savePickle:
    import pickle
    with open(outFolder+'conn_long_2.pkl', 'wb') as fileObj:        
...
if plotMat2D: ax=plotMats2D(labels=0, cbar=1)
